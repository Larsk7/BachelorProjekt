...
function filterSonsDt(array $dataPhase4): array {
    $resultData = [];

    // Arrays für 'IN'-Bedingungen
    $adbzExc = [
        '2882', '6720', '6730', '0570',        
   ...
   foreach ($dataPhase4 as $row) {
        ...
        $cond_adbz_ok = !in_array($adbz, $adbzExc);
        $cond_pbv_art_ok = !in_array($pbv_art, $pbvArtExc);

        if ($cond_adbz_ok && $cond_pbv_art_ok) {
            $resultData[] = $row;
        }
    }
    error_log("Nach Filterung (Sons-Dt A). Anzahl Zeilen: " . count($resultData));
